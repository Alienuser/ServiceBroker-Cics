...
# Get service information from on Bluemix
...
pseudo_service = {'id': pseudo_service_id,
                  'name': 'ServiceBroker-zOSMF',
                  ...
                  'metadata': {
                      'displayName': 'ServiceBroker-zOSMF',
                      'longDescription': 'really long description here like that this is a great showcase of brokers',
                      'providerDisplayName': 'data-henrik',
                      'documentationUrl': 'https://github.com/Alienuser/ServiceBroker-zOSMF',
...
def catalog():
    ...
    if not api_version or float(api_version) < X_BROKER_API_VERSION:
        abort(412, "Precondition failed. Missing or incompatible %s. Expecting version %0.1f or later" % (
...
def dashboard(instance_id):
    user = {'nickname': "Das ist ein Test"}
